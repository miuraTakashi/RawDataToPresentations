...
def convert_nd2_to_mp4(
    ...
    if _HAS_ND2READER:
        with ND2Reader(nd2_path) as images:  # type: ignore
            ...
            # Use FFmpeg for QuickTime-compatible encoding if requested and available
            if use_ffmpeg:
                print(f"Creating QuickTime-compatible MP4 with FFmpeg: {os.path.basename(out_path)}")
                if create_high_quality_mp4_with_ffmpeg(all_frames, out_path, fps):
                    print(f"✅ QuickTime-compatible MP4 created with FFmpeg: {os.path.basename(out_path)}")
                    return
                else:
                    print("⚠️  FFmpeg not available or failed, falling back to OpenCV VideoWriter")
            
            # Fallback to OpenCV VideoWriter
            print(f"Creating MP4 with OpenCV VideoWriter: {os.path.basename(out_path)}")
            h, w = all_frames[0].shape[:2]
            writer = ensure_video_writer(out_path, fps, (w, h), preferred_codec)
            for frame in all_frames:
                writer.write(frame)
            writer.release()
            print(f"✅ MP4 created with OpenCV: {os.path.basename(out_path)}")
            return
